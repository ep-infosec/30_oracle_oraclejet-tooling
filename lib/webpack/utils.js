...
const oracleJetPreactThemePath = path.join(oracleJetDistPath, 'js', 'libs', 'oraclejet-preact', 'amd');
...
const htmlTokenResources = {
  css: {
    ...
    preact: `./${configPaths.src.styles}/theme-redwood/theme.css`
  },
  injector: {
...
const preactThemePath = htmlTokenTEmplates.css.replace('%s', htmlTokenResources.css.preact);
...
function htmlTokenReplacementFunction(match, $1, type, file, $4, index, input) {
  ...
  // This ensures that preact link is injected into the index.html:
  const themeStyleLinks = `${htmlTokenTEmplates[type].replace('%s', url)}\n\t\t${preactThemePath}`;
  // $1==='@@' <--EQ--> $4===undefined
  return ($4 === undefined ? url : themeStyleLinks);
}
